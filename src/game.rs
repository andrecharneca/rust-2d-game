...
// Main loop
pub fn run_game() {
    let mut screen = Screen::new(WINDOW_WIDTH, WINDOW_HEIGHT);
    let mut player = Player::new("P".to_string(), (0.5, 0.5));
    let mut enemy = Enemy::new("E".to_string(), (0.1, 0.1));
    ...
    loop {
        ...
        enemy.move_pos((
            rand::thread_rng().gen_range(-1.0..1.0) * 0.01 as f32,
            rand::thread_rng().gen_range(-1.0..1.0) * 0.01 as f32
        ));
